...
// Inicialización de Supabase con seguridad
...
export default function App() {
  ...
  const loadData = async () => {
    setIsLoading(true);
    if (!supabase) {
      const saved = localStorage.getItem('dg_orders_v2');
    ...
    try {
      ...
      if (data) {
        ...
        setOrders(fetched);
        localStorage.setItem('dg_orders_v2', JSON.stringify(fetched));
  ...
  const syncOrder = async (order: Order) => {
    ...
    setOrders(prev => {
      ...
      localStorage.setItem('dg_orders_v2', JSON.stringify(newList));
      return newList;
    });

    // 2. Sincronización con Supabase en segundo plano
  ...
  useEffect(() => {
    loadData();
    if (supabase) {
      const channel = supabase.channel('dg_realtime_v2_main')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'orders' }, (payload) => {
          if (payload.eventType === 'INSERT') {
             setOrders(prev => {
               if (prev.some(o => o.id === payload.new.id)) return prev;
               return [payload.new.payload as Order, ...prev];
             });
    ...
    <div className="max-w-md mx-auto min-h-screen bg-slate-50 pb-24 relative font-sans overflow-x-hidden animate-custom">
      ...
      <header className="bg-white p-6 rounded-b-[48px] shadow-xl relative z-10 border-b-4 border-orange-500/10">
        ...
        <div className="text-center mt-2 flex items-center justify-center gap-2">
           <div className={`w-2 h-2 rounded-full ${dbStatus === 'connected' ? 'bg-green-500 animate-pulse' : 'bg-orange-400'}`} />
           <span className="text-[8px] font-black text-slate-300 uppercase tracking-[0.3em]">v2.0.0 - {dbStatus === 'connected' ? 'CLOUD SYNC' : 'OFFLINE'}</span>
        </div>
      </header>
      ...
      <main className="p-5 space-y-6">
        {isLoading && orders.length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <button onClick={() => setShowEntryModal(true)} className="col-span-2 bg-white border-2 border-slate-100 rounded-[36px] p-6 flex items-center justify-between shadow-sm active:scale-95 transition-all">
              <div className="flex items-center gap-4">
                <div className="bg-[#f97316] text-white p-3 rounded-2xl shadow-lg"><Plus size={24} /></div>
                <div>
                  <p className="font-black text-lg uppercase leading-none text-slate-800">NUEVA CARGA</p>
                  <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-1">Ingreso Manual de Pedidos</p>
                </div>
              </div>
          ...
          <div className="space-y-4 animate-in slide-in-from-bottom-4">
            ...
            <div className="space-y-3 pb-20">
              {filteredOrders.length === 0 ? (
                <div className="text-center py-20 opacity-20 font-black uppercase text-[10px] tracking-widest">Sin registros disponibles</div>
              ) : filteredOrders.map(order => (
                <div key={order.id} onClick={() => {
                  if (order.lockedBy && order.lockedBy !== currentUser.name) {
                    alert(`ESTE PEDIDO ESTÁ SIENDO EDITADO POR: ${order.lockedBy}`);
                    return;
                  }
                  const updated = { ...order, lockedBy: currentUser.name };
                  syncOrder(updated);
                  setSelectedOrder(updated);
                }} className={`bg-white border-2 rounded-[32px] p-5 shadow-sm relative active:scale-[0.98] transition-all cursor-pointer ${order.lockedBy ? 'border-orange-200 bg-orange-50/20' : 'border-slate-50'}`}>
                  <div className="flex justify-between items-start mb-1">
                    <p className="text-[10px] font-black text-orange-600 bg-orange-50 px-2 py-0.5 rounded-lg">#{order.orderNumber}</p>
                    {order.lockedBy && <div className="text-[7px] font-black text-orange-500 uppercase flex items-center gap-1 animate-pulse"><UserCheck size={10}/> EN CONTROL: {order.lockedBy}</div>}
                  </div>
                  <h3 className="font-black text-slate-800 text-lg uppercase tracking-tighter leading-none">{order.customerName}</h3>
                  <div className="mt-4 flex items-center justify-between">
                    <div className="text-[9px] font-black text-orange-700 bg-orange-50 px-3 py-1 rounded-full uppercase">
                      {order.detailedPackaging?.reduce((a,c) => a+c.quantity, 0) || 0} BULTOS
                    </div>
      ...
      {selectedOrder && (
        <OrderDetailsModal 
          ...
          onSave={(ord: Order) => { syncOrder(ord); showToast("Guardado con éxito"); }}
...
const StatCard = ({ count, label, color, icon, onClick }: any) => (
  <button onClick={onClick} className={`${color} rounded-[48px] p-7 text-white text-left shadow-2xl flex flex-col justify-between h-52 active:scale-95 transition-all overflow-hidden relative group`}>
    ...
    <div className="z-10">
      <h3 className="text-6xl font-black tracking-tighter leading-none">{count}</h3>
      <p className="text-[10px] font-black uppercase tracking-[0.25em] opacity-70 mt-4">{label}</p>
    </div>
...
const NavBtn = ({ active, icon, onClick }: any) => (
  <button onClick={onClick} className={`p-5 rounded-[28px] transition-all duration-300 ${active ? 'text-orange-500 bg-orange-50 shadow-inner scale-110' : 'text-slate-300 hover:text-slate-400'}`}>
    ...
    <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-md z-[150] flex items-end justify-center">
      <div className="bg-white w-full max-w-md rounded-t-[56px] p-6 space-y-6 max-h-[96vh] overflow-y-auto shadow-2xl border-t-[12px] border-orange-500 animate-in slide-in-from-bottom-10">
        <div className="flex justify-between items-start border-b pb-6 sticky top-0 bg-white z-20">
          <div>
            <h2 className="text-2xl font-black uppercase tracking-tighter text-slate-800 leading-tight">{order.customerName}</h2>
            <p className="text-[10px] font-black text-orange-600 bg-orange-50 px-3 py-1 rounded-full uppercase mt-2 w-fit">#{order.orderNumber}</p>
          </div>
        ...
        <section className="bg-slate-50 p-6 rounded-[48px] border border-slate-200 shadow-inner space-y-5 text-center">
          ...
          <div className="grid grid-cols-2 gap-4 text-left">
            <div className="space-y-2">
              ...
              <select className="w-full bg-white border-2 border-slate-200 p-4 rounded-3xl text-[11px] font-black outline-none focus:border-orange-400" value={loc} onChange={e => setLoc(e.target.value)}>
            ...
            <div className="space-y-2">
              ...
              <select className="w-full bg-white border-2 border-slate-200 p-4 rounded-3xl text-[11px] font-black outline-none focus:border-orange-400" value={type} onChange={e => setType(e.target.value)}>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => { if(qty<=0) return; setPkg([...pkg, { id: Date.now().toString(), deposit: loc, type: type, quantity: qty }]); setQty(1); }} className="flex-1 bg-orange-500 text-white font-black py-5 rounded-[32px] text-xs uppercase tracking-widest shadow-xl flex items-center justify-center gap-2 active:scale-95 transition-all">
          ...
          <div className="space-y-2 mt-4 max-h-48 overflow-y-auto pr-1 scrollbar-hide">
            {pkg.map(p => (
              <div key={p.id} className="bg-white p-4 rounded-3xl flex justify-between items-center border border-slate-100 shadow-sm animate-in zoom-in-95">
        ...
        <div className="flex flex-col gap-4 pb-12 pt-4 border-t">
          ...
          {order.status === OrderStatus.COMPLETED && (
            <button onClick={() => onStatusUpdate(OrderStatus.DISPATCHED, { detailedPackaging: pkg })} className="w-full bg-orange-600 text-white font-black py-7 rounded-[32px] uppercase text-xs tracking-widest border-b-8 border-orange-800 shadow-xl active:scale-95 transition-all">ENVIAR A DESPACHO 🚛</button>
          )}
        </div>
      </div>
    </div>
    ...
    <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-2xl z-[200] flex items-center justify-center p-6">
      <div className="bg-white w-full max-w-md rounded-[64px] shadow-2xl p-10 space-y-8 animate-in zoom-in-95">
        <div className="text-center space-y-1">
          ...
          <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Ingrese los datos para control</p>
        </div>
        <div className="space-y-3">
          <input className="w-full bg-slate-50 border-2 border-slate-100 rounded-3xl p-5 font-black text-xs uppercase outline-none focus:border-orange-500 shadow-inner" placeholder="NRO DE PEDIDO" value={n} onChange={(e) => setN(e.target.value.toUpperCase())} />
          <input className="w-full bg-slate-50 border-2 border-slate-100 rounded-3xl p-5 font-black text-xs uppercase outline-none focus:border-orange-500 shadow-inner" placeholder="CLIENTE / EMPRESA" value={c} onChange={(e) => setC(e.target.value.toUpperCase())} />
          <input className="w-full bg-slate-50 border-2 border-slate-100 rounded-3xl p-5 font-black text-xs uppercase outline-none focus:border-orange-500 shadow-inner" placeholder="LOCALIDAD DESTINO" value={l} onChange={(e) => setL(e.target.value.toUpperCase())} />
          <button onClick={() => { if(!c || !n) return; onAdd(c, l, n); }} className="w-full bg-orange-600 text-white font-black py-6 rounded-[32px] uppercase text-sm tracking-[0.2em] shadow-xl active:scale-95 transition-all flex items-center justify-center gap-3">INICIAR PROCESO <Send size={20}/></button>
          <button onClick={onClose} className="w-full text-slate-400 font-black text-[10px] uppercase tracking-[0.4em] text-center mt-4">Cancelar Operación</button>
        </div>
      </div>
    </div>
  );
...
const CustomerPortal = ({ onBack, allOrders, showToast }: any) => {
  ...
  const track = () => { 
    if(!n) return;
    const o = allOrders.find((x: Order) => x.orderNumber.toUpperCase().trim() === n.toUpperCase().trim()); 
    setF(o || null); 
    if(!o) showToast("No se encontró el pedido", "error"); 
  };
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 p-8 flex flex-col items-center py-16 space-y-12">
      ...
      <div className="bg-slate-50 rounded-[56px] p-10 w-full max-w-md space-y-6 shadow-2xl border-t-8 border-[#f97316]">
        <div className="text-center space-y-1">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">Seguimiento de Envíos</p>
        </div>
        <input className="w-full bg-white p-6 rounded-[32px] text-slate-900 font-black uppercase text-center border-2 border-slate-100 outline-none focus:border-orange-500 text-lg shadow-inner" placeholder="INGRESE NRO PEDIDO" value={n} onChange={e => setN(e.target.value)} />
        <button onClick={track} className="w-full bg-[#f97316] py-7 rounded-[32px] font-black uppercase text-xs tracking-[0.3em] text-white flex items-center justify-center gap-3 active:scale-95 transition-all shadow-xl"><ScanSearch size={28}/> CONSULTAR ESTADO</button>
      </div>
      {f && (
        <div className="bg-white border-2 border-slate-50 rounded-[64px] p-10 w-full max-w-md text-slate-900 space-y-8 animate-in zoom-in-95 shadow-2xl">
          ...
          <div className="bg-orange-50 p-10 rounded-[48px] text-center shadow-inner relative overflow-hidden">
             <p className="text-[11px] font-black text-orange-400 uppercase tracking-[0.4em] mb-2">BULTOS LISTOS</p>
      ...
      <button onClick={onBack} className="text-slate-400 text-[10px] font-black uppercase flex items-center gap-2 hover:text-orange-600 transition-colors tracking-widest"><ArrowLeft size={16}/> REGRESAR</button>
    </div>
  );
};
  ...
  return (
    <div className="fixed inset-0 bg-slate-950 z-[1000] flex items-center justify-center p-8">
      <div className="bg-white w-full max-w-xs rounded-[72px] p-12 text-center space-y-12 shadow-2xl border-t-8 border-orange-500">
        ...
        <button onClick={onClientPortal} className="text-[#f97316] font-black text-[10px] uppercase tracking-[0.4em] hover:text-orange-400 transition-colors block mx-auto">Portal para Clientes</button>
      </div>
    </div>
  );
};
