...
export default function App() {
  ...
  // FIX: Added missing state for General Entry Modal visibility
  ...
  useEffect(() => {
    loadData();
    if (supabase) {
      const channel = supabase.channel('realtime_prod_v15')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'orders' }, (payload) => {
          if (payload.eventType === 'INSERT') {
            setOrders(prev => [payload.new.payload as Order, ...prev]);
            showToast("📦 NUEVA CARGA ENTRANTE");
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", `reporte_dg_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const syncOrder = async (order: Order) => {
    if (supabase && dbStatus === 'connected') {
      ...
      if (error) showToast("Error de sincronización", "error");
    } else {
      setOrders(prev => {
    ...
    <div className="max-w-md mx-auto min-h-screen bg-slate-50 pb-24 relative font-sans overflow-x-hidden">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="text-center">
            ...
            <div className="flex items-center justify-center gap-2 mt-1">
              <span className="text-[6px] font-black bg-indigo-500 px-1.5 py-0.5 rounded text-white">v1.5 PROD</span>
      ...
      <main className="p-5 space-y-6">
        {isLoading && orders.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 opacity-20"><Database className="animate-bounce mb-4" size={48} /><p className="text-[10px] font-black uppercase tracking-[0.3em]">Cargando DG_LOGISTICA...</p></div>
        ) : view === 'DASHBOARD' ? (
          <div className="grid grid-cols-2 gap-4">
            ...
            {currentUser.role === 'admin' && (
              <button onClick={exportToCSV} className="col-span-2 bg-slate-100 text-slate-600 rounded-[32px] p-5 flex items-center justify-center gap-3 font-black text-[10px] uppercase tracking-[0.2em] hover:bg-slate-200 transition-colors">
          ...
          <div className="space-y-4">
            ...
            <div className="relative">
              ...
              <input type="text" placeholder="Buscar cliente o pedido..." className="w-full bg-white border-2 border-slate-100 rounded-3xl py-4 pl-12 pr-4 text-sm font-bold outline-none focus:border-indigo-500 transition-all" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
            </div>
            <div className="space-y-3 pb-20">
              {filteredOrders.map(order => (
                <div key={order.id} onClick={() => {
                  ...
                  <h3 className="font-black text-slate-800 text-lg uppercase tracking-tighter">{order.customerName}</h3>
    ...
    <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-md z-[150] flex items-end justify-center">
      <div className="bg-white w-full max-w-md rounded-t-[56px] p-6 space-y-6 max-h-[96vh] overflow-y-auto shadow-2xl border-t-[12px] border-indigo-600 animate-in slide-in-from-bottom-10">
        ...
        <section className="bg-slate-50 p-6 rounded-[48px] border border-slate-200 shadow-inner space-y-5 text-center">
          ...
          <span className="text-9xl font-black text-indigo-600 drop-shadow-lg">{total}</span>
          ...
          <div className="grid grid-cols-2 gap-4 text-left">
            <div className="space-y-2">
              ...
              {loc === 'OTRO' && <input className="w-full bg-white border-2 border-indigo-200 p-4 rounded-3xl text-[11px] font-bold" placeholder="¿Dónde?" value={custLoc} onChange={e => setCustLoc(e.target.value.toUpperCase())}/>}
            </div>
            <div className="space-y-2">
              ...
              {type === 'OTRO' && <input className="w-full bg-white border-2 border-indigo-200 p-4 rounded-3xl text-[11px] font-bold" placeholder="¿Qué es?" value={custType} onChange={e => setCustType(e.target.value.toUpperCase())}/>}
            </div>
          </div>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => { if(qty<=0) return; setPkg([...pkg, { id: Date.now().toString(), deposit: loc === 'OTRO' ? custLoc : loc, type: type === 'OTRO' ? custType : type, quantity: qty }]); }} className="flex-1 bg-indigo-600 text-white font-black py-5 rounded-[32px] text-xs uppercase tracking-widest shadow-xl flex items-center justify-center gap-2"><Plus size={18}/> AGREGAR BULTOS</button>
          </div>
...
const StatCard = ({ count, label, color, icon, onClick }: any) => (
  <button onClick={onClick} className={`${color} rounded-[48px] p-7 text-white text-left shadow-2xl flex flex-col justify-between h-52 active:scale-95 transition-all overflow-hidden relative group`}>
    <div className="z-10 bg-white/10 p-4 rounded-2xl self-start transition-colors">{icon}</div>
...
const NavBtn = ({ active, icon, onClick }: any) => (
  <button onClick={onClick} className={`p-5 rounded-[28px] transition-all duration-300 ${active ? 'text-indigo-600 bg-indigo-50 shadow-inner scale-110' : 'text-slate-300'}`}>{React.cloneElement(icon, { size: 28 })}</button>
);
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-2xl z-[200] flex items-center justify-center p-6">
      <div className="bg-white w-full max-w-md rounded-[64px] shadow-2xl p-12 space-y-8 animate-in zoom-in-95 duration-300">
        <h2 className="font-black text-3xl uppercase tracking-tighter text-slate-800 text-center leading-none">NUEVA CARGA</h2>
        <div className="space-y-4">
          ...
          <button onClick={() => { if(!c || !n) return; onAdd(c, l, n); }} className="w-full bg-slate-900 text-white font-black py-6 rounded-[32px] uppercase text-sm tracking-[0.2em] shadow-xl active:scale-95 transition-all flex items-center justify-center gap-3">INICIAR CONTROL <Send size={22}/></button>
...
const CustomerPortal = ({ onBack, allOrders }: any) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-8 flex flex-col items-center py-20 space-y-12">
      <div className="text-center space-y-3"><h1 className="text-5xl font-black tracking-tighter uppercase leading-none">D&G BAZAR</h1><p className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.6em]">Seguimiento Real</p></div>
    ...
    <div className="fixed inset-0 bg-slate-950 z-[1000] flex items-center justify-center p-8">
      <div className="bg-white w-full max-w-xs rounded-[72px] p-12 text-center space-y-12 shadow-2xl border-t-8 border-indigo-600">
        ...
        <form onSubmit={e => { e.preventDefault(); onLogin({ name: u, role: u.toLowerCase() === 'admin' ? 'admin' : 'staff' }); }} className="space-y-6">
          <input className="w-full bg-slate-50 p-6 rounded-[32px] text-sm font-bold text-center border-2 border-slate-50 uppercase focus:border-indigo-500 outline-none shadow-inner" placeholder="TU NOMBRE" value={u} onChange={e => setU(e.target.value)} required />
        ...
        <button onClick={onClientPortal} className="text-indigo-600 font-black text-[10px] uppercase tracking-[0.4em] hover:text-indigo-400 transition-colors">Seguimiento Clientes</button>
      </div>
    </div>
  );
};
